...
def pdf_to_text(pdf):
    ...
    :return: If a pdf filename matches the input, a .txt file is derived and the pdf is then deleted.
    """
...
def convert_time(time_str):
    """
    :param time_str: (string) 'H:MM' or 'HH:MM'
    :return: (string) 'HH MM'
    """
    if len(time_str)== 0 or time_str == 'Null':
        return 'Null'
    else:
    ...
    assert(formatted[2].isspace() and int(formatted[:2]) <= 23)
...
def create_daily_booking_list():
    """
    :details: Get the link, download te pdf, convert to .txt, parse it, fix the date/time item
